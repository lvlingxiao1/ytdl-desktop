...
async function streamToFile(stream: ReadableStream, filePath: string) {
	...
	while (true) {
		...
		await invoke('append_chunk_to_file', {
			path: filePath,
			base64: base64Chunk,
		});
	}

	console.log('Stream written to file!');
}
		...
		html`<h3>${info.videoDetails.title}</h3>
			<table>
				<tbody>
					...
					${info.formats
						.filter((i) => (i.hasAudio || !require_audio.checked) && (i.hasVideo || !require_video.checked))
						.map(
							(i) => html`<tr>
								<td>${i.container}</td>
								<td>${i.hasAudio ? '√' : ''}</td>
								...
								<td id="download-${i.itag}">
									<button @click=${(e) => download(i)}>Download</button>
								</td>
							</tr> `
						)}
				</tbody>
